...
###관련도 점수: 0~100 (AIMSP사업과의연관성)                                      
즉, Gemini가 기사를 분석할 때 "이 기사가 AI MSP 사업 전략(AI Agent, ITO 전환, MSP 서비스 등) 관점에서 얼마나 유용한
정보인가"를 0~100으로 평가한 점수입니다.
예를 들어:
  - 높은 점수 (80~100): LG CNS가 AI Agent 기반 ITO 계약 수주 → 직접적 경쟁 정보
  - 중간 점수 (40~70): 클라우드 시장 전반 동향 기사 → 간접적 참고 정보
  - 낮은 점수 (0~30): 경쟁사가 언급되지만 AI/MSP와 무관한 내용 → 참고 가치 낮음